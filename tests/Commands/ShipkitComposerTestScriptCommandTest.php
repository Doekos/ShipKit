...
it('adds composer scripts to composer.json', function (): void {
    $this->artisan('shipkit:composer', ['--force' => true])
        ->assertExitCode(0)
        ->assertSuccessful();
    ...
    expect($composerJson)->toHaveKey('scripts')
        ...
        ->and($composerJson['scripts'])->toHaveKey('fix');
});

it('creates a backup when backup option is provided', function (): void {
    $this->artisan('shipkit:composer', ['--force' => true, '--backup' => true])
        ->assertExitCode(0)
        ->assertSuccessful();
